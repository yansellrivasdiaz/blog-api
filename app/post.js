...
function fillposts(post){
    var userdata = JSON.parse(localStorage.getItem("blogapi"));
    var url = 'http://68.183.27.173:8080/users/'+post.userId;
    var cabecera = new Headers();
    cabecera.append("Authorization",'Bearer '+ userdata.token);                    
    cabecera.append('Content-Type', 'application/json');
    var init = {
    ...
    fetch(myrequest).then(function(response){
        ...
        <h5 class="card-title"><a href="javascript:void(0)"><b class="post-title">${post.title}</b> <i class="fas fa-share-square ml-2"></i></a></h5>
                <div class="like py-0">
                    <a class="btn btn-link text-info py-0 my-0 btn-likepost" href="javascript:void(0);" data-postid="${post.id}"><i class="far fa-star fa-lg"></i></a> 
                ...
                <blockquote class="blockquote-title text-truncate">                                        
                    <a class="btn btn-link text-info float-right readmore" href="javascript:void(0);" data-postid="${post.id}"><i class="far fa-eye"></i> Leer más...</a>  
                    ...
                    <blockquote class="blockquote-footer">
                        <a class="text-success mb-2" href="javascript:void(0);" data-ownerid="${data.id}">
                            <i class="fa fa-user"></i> 
                            <b>By:</b> 
                            <b class="post-owner">${data.name} (${data.email})</b>
                        </a> 
                        <!-- <em class="postdate">05 febrero, 2018</em> -->
                    </blockquote>
                </blockquote>  
        ...
        $("#comment-container").append(htmlpost);
    }).catch(function(error){
        alertshow("Hubo problema con la peticion fetch"+error.message,"danger");
    });
}